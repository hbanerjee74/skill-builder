...
describe("skillsUsed tracking", () => {
  beforeEach(() => {
    useAgentStore.getState().clearRuns();
  });

  it("startRun initializes skillsUsed as empty array", () => {
    useAgentStore.getState().startRun("agent-1", "sonnet");
    const run = useAgentStore.getState().runs["agent-1"];
    expect(run.skillsUsed).toEqual([]);
  });

  it("detects Skill tool_use in assistant messages and adds to skillsUsed", () => {
    useAgentStore.getState().startRun("agent-1", "sonnet");
    ...
    const run = useAgentStore.getState().runs["agent-1"];
    expect(run.skillsUsed).toEqual(["shadcn-ui"]);
  });

  it("deduplicates skills when same skill is invoked multiple times", () => {
    useAgentStore.getState().startRun("agent-1", "sonnet");

    const makeSkillMsg = (skill: string): AgentMessage => ({
      type: "assistant",
      content: null as unknown as string,
      raw: {
        message: {
    ...
    useAgentStore.getState().addMessage("agent-1", makeSkillMsg("shadcn-ui"));
    useAgentStore.getState().addMessage("agent-1", makeSkillMsg("shadcn-ui"));
    useAgentStore.getState().addMessage("agent-1", makeSkillMsg("tailwind"));
    flushMessageBuffer();

    const run = useAgentStore.getState().runs["agent-1"];
    expect(run.skillsUsed).toEqual(["shadcn-ui", "tailwind"]);
  });

  it("does not add skills from non-Skill tool_use blocks", () => {
    useAgentStore.getState().startRun("agent-1", "sonnet");
  ...
  it("does not add skills from non-assistant message types", () => {
    useAgentStore.getState().startRun("agent-1", "sonnet");
