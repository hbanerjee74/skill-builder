...
For M+ or multi-component work, create a short plan first. Use sub-agents only when parallel planning/research is beneficial.

Inputs: worktree path, issue title, requirements, acceptance criteria.
...
If the user rejects the plan, revise it with the user's feedback. The revised plan must present 2-3 alternative approaches with trade-offs (scope, complexity, risk). If the chosen approach changes requirements or ACs, update the Linear issue via `mcp__linear__save_issue` before execution.
