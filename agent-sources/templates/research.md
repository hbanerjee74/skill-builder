...
**Both agents relevant**: Spawn `{{NAME_PREFIX}}-research-entities` and `{{NAME_PREFIX}}-research-metrics` in parallel. When both return, spawn `consolidate-research` (`model: "opus"`) to merge their output into cohesive concept text. Pass it:
