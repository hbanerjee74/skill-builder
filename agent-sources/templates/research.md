...
- **Entity agent** (`{{NAME_PREFIX}}-research-entities`): Launch if the domain involves modeling entities with relationships (e.g., business objects, resources, API objects, pipeline components).
- **Metrics agent** (`{{NAME_PREFIX}}-research-metrics`): Launch if the domain involves KPIs, calculations, measurements, or performance indicators.
...
**Both agents relevant**: Spawn `{{NAME_PREFIX}}-research-entities` and `{{NAME_PREFIX}}-research-metrics` in parallel. When both return, combine their text under clear section headers (`## Entity & Relationship Research` and `## Metrics & KPI Research`). This combined text is the concept text for Phase 2.
...
Spawn `{{NAME_PREFIX}}-research-practices` and `{{NAME_PREFIX}}-research-implementation` in parallel.
