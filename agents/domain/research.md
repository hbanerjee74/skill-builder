...
- **Entity agent** (`domain-research-entities`): Relevant if the domain involves modeling entities with relationships (e.g., business objects, resources, API objects, pipeline components). Skip for domains with no meaningful entity modeling.
- **Metrics agent** (`domain-research-metrics`): Relevant if the domain involves KPIs, calculations, measurements, or performance indicators. Skip for domains with no quantitative dimension.
- **Practices agent** (`domain-research-practices`): Relevant if the domain has meaningful patterns, edge cases, or industry-specific variations. Skip for narrow, well-defined domains with no ambiguity.
- **Implementation agent** (`domain-research-implementation`): Relevant if the domain involves technical implementation decisions (architectures, frameworks, deployment patterns). Skip for purely conceptual domains.
