---
# AUTO-GENERATED — do not edit. Source: agent-sources/templates/research.md + agent-sources/types/platform/config.conf
...
- **Entity agent** (`platform-research-entities`): Launch if the domain involves modeling entities with relationships (e.g., business objects, resources, API objects, pipeline components). Skip for domains with no meaningful entity modeling (e.g., pure algorithms, simple CLI tools).
- **Metrics agent** (`platform-research-metrics`): Launch if the domain involves KPIs, calculations, measurements, or performance indicators. Skip for domains with no quantitative dimension (e.g., pure configuration, text processing).

**Exploratory agents** (Phase 2 — run after foundational, receive concept text as context):
- **Practices agent** (`platform-research-practices`): Launch if the domain has meaningful patterns, edge cases, or industry-specific variations that affect skill design. Skip for narrow, well-defined domains with no ambiguity.
- **Implementation agent** (`platform-research-implementation`): Launch if the domain involves technical implementation decisions (architectures, frameworks, deployment patterns). Skip for domains that are purely conceptual or non-technical.
