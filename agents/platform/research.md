...
- **Entity agent** (`platform-research-entities`): Launch if the domain involves modeling entities with relationships (e.g., business objects, resources, API objects, pipeline components).
- **Metrics agent** (`platform-research-metrics`): Launch if the domain involves KPIs, calculations, measurements, or performance indicators.
...
**Both agents relevant**: Spawn `platform-research-entities` and `platform-research-metrics` in parallel. When both return, combine their text under clear section headers (`## Entity & Relationship Research` and `## Metrics & KPI Research`). This combined text is the concept text for Phase 2.
...
Spawn `platform-research-practices` and `platform-research-implementation` in parallel.
