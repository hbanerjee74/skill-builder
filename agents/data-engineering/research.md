...
- **Entity agent** (`de-research-entities`): Launch if the domain involves modeling entities with relationships (e.g., business objects, resources, API objects, pipeline components).
- **Metrics agent** (`de-research-metrics`): Launch if the domain involves KPIs, calculations, measurements, or performance indicators.
...
**Both agents relevant**: Spawn `de-research-entities` and `de-research-metrics` in parallel. When both return, combine their text under clear section headers (`## Entity & Relationship Research` and `## Metrics & KPI Research`). This combined text is the concept text for Phase 2.
...
Spawn `de-research-practices` and `de-research-implementation` in parallel.
