...
- **Entity agent** (`source-research-entities`): Launch if the domain involves modeling entities with relationships (e.g., business objects, resources, API objects, pipeline components).
- **Metrics agent** (`source-research-metrics`): Launch if the domain involves KPIs, calculations, measurements, or performance indicators.
...
Spawn `source-research-practices` and `source-research-implementation` in parallel.
